...
different architectures (Intel Xeon Phi and AMD GPUs, in addition 
...
those studies to include other many-core architectures (Intel MIC in
addition to GPUs) and other programming toolsets (\textsc{opencl}).
...
(SVT) system at CDF~\cite{SVT1}.  We also investigate the performance
of the algorithm on a GPU manufactured by AMD using a different
computing environment: \textsc{opencl}.  Starting with a serial
algorithm implemented on a CPU, we test an \textit{embarrassingly
...
To be run on NVidia GPUs, the code has been ported to \textsc{cuda}:
...
To implement the algorithm to run on an AMD GPU, we have ported the
...
class GPUs (the NVidia GTX and AMD Radeon cards) and ones optimized
...
\begin{table*}[tbp]
  \caption{Capabilities of the many-core devices used in this study,
    ...
    the final one is an AMD GPU.  For Xeon Phi, the ``cores'' column
...
use of the multi-core architecture. Additionally, we have an AMD
...
are only compatible with Nvidia GPUs. Additionally, \textsc{opencl}
...
\textsc{opencl} algorithm on the AMD Radeon GPU. The \textsc{opencl}
...
we are unable to run the same \textsc{opencl} code on Nvidia GPUs. On two
different installations with two different video cards, we find
incorrect results running the code that ran successfully on the AMD
